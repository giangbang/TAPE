...
class OffPGLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.mixer_optimiser = Adam(params=self.mixer_params, lr=args.critic_lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, log):
        ...
        baseline = th.sum(mac_out * q_vals, dim=-1).detach()
        ...
        adv=advantages*coe.view(advantages.shape)
        adv=adv.reshape(advantages.shape[0],advantages.shape[1],1,advantages.shape[2]).repeat(1,1,advantages.shape[2],1)

        A=th.FloatTensor([[1 if np.random.uniform()<self.args.p else 0 for _ in range(self.n_agents)]for _ in range(self.n_agents)]).cuda()
        for i in range(self.n_agents):
            A[i,i]=1.0
        adjcent=A.reshape(1,1,A.shape[0],-1).repeat(adv.shape[0],adv.shape[1],1,1)
        adv=(adv*adjcent).sum(-1).view(-1)
        
        coma_loss = - ((adv * log_pi_taken) * mask).sum() / mask.sum()
